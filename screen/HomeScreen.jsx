// Note: Styles are now merged at the bottom.
...
// Imports moved from ShayariFeedScreen
...
import NativeCard from "../NativeCardAds"; // Assuming this is the correct path
...
// Card dimensions from ShayariFeedScreen
...
const HomeScreen = () => {
  ...
  // State moved from ShayariFeedScreen
  ...
  const fetchShayaris = useCallback(async (pageNum) => {
    if (!hasMore || isFetchingMore) return;
    if (pageNum > 1) setIsFetchingMore(true); else setLoading(true);
  ...
  // FAVORITES LOGIC (from ShayariFeedScreen)
  ...
  // CARD ACTIONS (from ShayariFeedScreen)
  ...
  const handleEdit = useCallback((item) => {
    navigation.navigate("HomeStack", { // Assuming 'HomeStack' is the name of your stack navigator
  ...
  // SHARE MODAL ACTIONS (from ShayariFeedScreen)
  const shareAsImage = async () => {
    if (!captureViewRef.current) return;
    try {
      const uri = await captureRef(captureViewRef.current, { format: "png", quality: 1 });
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, { mimeType: "image/png", dialogTitle: "Share Shayari Image" });
  ...
  const saveToGallery = async () => {
    ...
    try {
      ...
      if (!granted) {
        Toast.show("Permission to access gallery is required.");
        return;
      };
      const uri = await captureRef(captureViewRef.current, { format: "png", quality: 1 });
    ...
    <View style={styles.cardWrapper}>
      <TouchableOpacity
        ...
        onPress={() => {
          ...
          if (categoryTapCountRef.current % 3 === 0) showRewardAd();
  ...
  ), [navigation, theme.card, theme.text, showRewardAd]);

  // *** NEW ShayariCard Component ***
      ...
      <View style={[styles.shayariCard, { backgroundColor }]}>
        ...
        <View style={styles.iconBar}>
          <TouchableOpacity onPress={() => handleCopy(item)}>
            {isCopied ? <TickIcon width={40} height={40} fill={textColor} /> : <CopyIcon width={40} height={40} fill={textColor} />}
          </TouchableOpacity>
          <TouchableOpacity onPress={() => handleEdit(item)}>
            <EditIcon width={40} height={40} fill={textColor} />
          </TouchableOpacity>
          <TouchableOpacity onPress={() => toggleFavorite(item)}>
            {isFavorite ? <LikedIcon width={22} height={20} fill="#E91E63" /> : <FavIcon width={40} height={40} fill={textColor} />}
          </TouchableOpacity>
      ...
      case 'categories':
        return (
          <FlashList
            ...
            scrollEnabled={true} // Allow horizontal scroll
            renderItem={renderCategory}
            estimatedItemSize={cardSize + 14} // width + margin
          />
        );
      case 'writeBox':
        return (
          <View style={styles.writeBox}>
            ...
            >
              <View style={styles.iconWrapper}>
                <Image source={require("../assets/pencil 2.png")} style={styles.pencilIcon} />
              </View>
  ...
  }, [theme, navigation, renderCategory, quickLinks, isLogin, userId, favorites, copiedId, getSpotlightShayaris]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      <FlashList
        ...
        ListFooterComponent={() =>
          isFetchingMore ? <ActivityIndicator size="small" color="#999" style={{ marginVertical: 20 }} /> : null
        }
        contentContainerStyle={{ paddingHorizontal: scale(10), paddingBottom: insets.bottom + 60 }} // padding for banner ad
      />
        ...
        <Modal visible transparent animationType="fade">
          <View style={styles.modalOverlay}>
            <View style={styles.modalBox}>
              ...
              >
                <CrossXMark width={24} height={24} fill="#000" />
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  writeHeading: { color: "#ffffff", fontSize: scaleFont(20), textAlign: "center", lineHeight: scaleFont(26), fontFamily: "Manrope_500Medium", paddingHorizontal: 10 },
